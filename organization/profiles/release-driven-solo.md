...
tastile-brands の current release-driven development で使用する Operating Model を定義します。
...
- GitHub Issues / Pull Requests を durable work / review surface とする
- 未採用の Linear をこの reconciliation だけで canonical 化しない
- implementation / dependency facts は GitHub Issues 側の ownership を維持する
...
- standard bootstrap: `mise run skills-bootstrap` / `bunx skills add rebuildup/project-init --skill '*' --agent claude-code --agent codex -y`
- canonical install layout: `.agents/skills/` + `.claude/skills/`
- continuous update: `mise run skills-update` / `bunx skills update -p -y`
- `skills-lock.json` is committed source / skill-path / content-hash metadata.
- `skills install` is not the sole fresh-clone path because current CLI restore is universal-path-only.
